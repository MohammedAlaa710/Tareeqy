...
print("hi: authentication token : "+authenticationToken);
  ...
  final String baseUrl = "https://accept.paymobsolutions.com/api/acceptance/transactions/";
  final Dio _dio = Dio();

 Future<Map<String, dynamic>> _getTransaction(String authToken, String orderId) async {
  try {
    print('hi: Starting getTransaction');
    
    // Step 1: Inquire transaction by orderId
    print('hi: Inquiry transaction by orderId: $orderId');
    Map<String, dynamic> inquiryResponse = await _inquireTransactionByOrderId(authToken, orderId);
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load transaction. Status code: ${response.statusCode}');
    }
  } catch (e) {
  ...
  Future<Map<String, dynamic>> _inquireTransactionByOrderId(String authToken, String orderId) async {
  try {
    print('hi: Starting inquireTransactionByOrderId order id is $orderId and the auth token is $authToken');
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to inquire transaction. Status code: ${response.statusCode}');
    }
  } catch (e) {
...
Future<void> navigateToPaymobView(BuildContext context, TextEditingController _amountController) async {
  try {
    int amount = int.parse(_amountController.text);
    Map<String, dynamic> paymentKeyResponse = await PaymobManager()._getPayment(
      amount,
      "EGP",
    );
          ...
          return Scaffold(
            ...
            body: InAppWebView(
              shouldOverrideUrlLoading: (controller, navigationAction) async {
                ...
                if (url.toString().contains("https://accept.paymob.com/api/acceptance/iframes/851752?payment_token=$_paymentKey")) {
                  return NavigationActionPolicy.CANCEL;
                }

                return NavigationActionPolicy.ALLOW;
              },
              initialUrlRequest: URLRequest(url: Uri.parse("https://accept.paymob.com/api/acceptance/iframes/851752?payment_token=$_paymentKey")),
