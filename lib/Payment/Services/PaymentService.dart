...
class PaymentService {
  ...
  Future<void> addAmountToUserWallet(BuildContext context, String amount) async {
    try {
      ...
      DocumentReference walletRef = _firestore.collection('users').doc(user.uid);
      ...
      if (walletSnapshot.exists && walletSnapshot.data() != null) {
        ...
        if (walletData.containsKey('wallet')) {
          currentAmount = double.tryParse(walletData['wallet'].toString()) ?? 0.0;
        }
      }
