...
class AdminService {
  ...
  Future<void> addStation(String line, String name, int number, GeoPoint latlng) async {
    final DocumentSnapshot doc = await _firestore.collection(line).doc(name).get();
    if (doc.exists) {
  ...
  Future<void> updateStation(String line, String name, int number, GeoPoint latlng) async {
    final DocumentSnapshot doc = await _firestore.collection(line).doc(name).get();
    if (!doc.exists) {
...
//////////////////////////////////      BUS Services
 Future<void> addBus(String busNumber, List<String> regions) async {
  ...
  final DocumentSnapshot busDoc = await _firestore.collection('Bus2').doc(busNumber).get();
  if (busDoc.exists) {
  ...
  for (String region in regions) {
    final DocumentSnapshot regionDoc = await _firestore.collection('Bus').doc(region).get();
    if (!regionDoc.exists) {
      // If region document doesn't exist, create a new one
      await _firestore.collection('Bus').doc(region).set({
        'Bus_Number': [busNumber], // Initialize Bus_Number array with the busNumber
      });
    } else {
...
Future<void> updateBus(String oldBusNumber, String newBusNumber, List<String> newRegions) async {
  // Get current regions of the bus
  final DocumentSnapshot busDoc = await _firestore.collection('Bus2').doc(oldBusNumber).get();
  if (!busDoc.exists) {
    throw Exception("Bus with this number does not exist");
  }
  final List<String> currentRegions = List<String>.from(busDoc['Regions']);
  ...
  // Update the regions in the Bus collection
  for (String region in currentRegions) {
    if (!newRegions.contains(region)) {
      ...
      final DocumentSnapshot updatedRegionDoc = await _firestore.collection('Bus').doc(region).get();
      if (updatedRegionDoc.exists && (updatedRegionDoc['Bus_Number'] as List).isEmpty) {
  ...
  for (String newRegion in newRegions) {
    if (!currentRegions.contains(newRegion)) {
      final DocumentSnapshot regionDoc = await _firestore.collection('Bus').doc(newRegion).get();
      if (!regionDoc.exists) {
        // If the region document doesn't exist, create a new one
        await _firestore.collection('Bus').doc(newRegion).set({
          'Bus_Number': currentRegions.contains(newRegion) ? [newBusNumber, ...regionDoc['Bus_Number']] : [newBusNumber],
        });
      } else {
        // If the region document exists, update it with the new bus number
        final List<dynamic> busNumbers = List<dynamic>.from(regionDoc['Bus_Number']);
  ...
  List<DocumentSnapshot> filterBusesByNumber(String value, List<DocumentSnapshot> buses) {
